...
- ⚠️ **这条路只有 Kevin 走得通。** `main` 有分支保护（要求至少 1 个 Approve），保护对管理员不生效、对其他人生效 —— 队友改文档请走 §4 的 PR。
...
8. `main` **是受保护分支**（2026-09-22 核实）：**要求至少 1 个 Approve**、**禁止强推**、**禁止删除该分支**；未配置强制状态检查，CI 绿不绿靠自觉看。保护对管理员不生效，因此 **Kevin 可以直接推送，其余人不能** —— 也就是说 §3 那条"文档小改可直接提交 `main`"**只对 Kevin 成立**，队友改文档同样要走 PR。规则外的情况请默认走分支。
9. **PR 描述里写清两件事**：这次**改了什么**、**为什么改**。改动源自需求调整时，说明它替代了原来的什么做法 —— 这就是留痕，不需要为此先去改文档再开工。
