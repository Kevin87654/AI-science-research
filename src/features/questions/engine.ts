...
export function answerQuestion(question: unknown, catalog: Catalog, knowledge: Knowledge): Answer {
   ...
   * 代写与提示注入（产品红线）。
   *
   * ⚠️ 2026-09-23 补了「帮我写…论文」这一族：原来只认 `代写|伪造|编造|虚构`，
   * 于是「**帮我写一篇论文**」会落到通用兜底（「已有资料无法确认这个问题」）——
   * 对一条贴着红线的请求来说，那个回答太弱了。
   *
   * **刻意写成吸附式**（`帮[我你]?写(一篇|个|份)?论文`）而不是 `帮.*写.*论文`：
   * 后者会把「帮我写封邮件问论文的事」这类**正当请求**也误伤掉。
   * 吸附式要求"写"和"论文"之间只允许出现一个量词，所以：
   *   ✓ 帮我写一篇论文 / 帮我写论文 / 替我写个论文
   *   ✗ 帮我写封邮件问论文（中间隔了"封邮件"）
   */
  if (
    /代写|伪造|编造|虚构|帮[我你]?写(一篇|个|份)?论文|替[我你]?写(一篇|个|份)?论文|忽略.*规则|ignore.*instruction|system prompt|系统提示词/.test(
      q,
    )
  ) {
    return unknownAnswer(
